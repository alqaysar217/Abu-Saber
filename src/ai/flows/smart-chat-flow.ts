...
2. الثقة والدقة: لا تعتذر عن نقص البيانات إذا كانت موجودة في السياق. ابحث جيداً في "recentSales" و "recentPurchases" وفي حقل "items" داخل كل منهما.
3. الأرقام الإنجليزية: استخدم الأرقام الإنجليزية (1, 2, 3...) دائماً في إجاباتك وداخل الجداول.
4. الذكاء السياقي: إذا سألك المستخدم عن تفاصيل فاتورة معينة (مثل P-0004)، ابحث عنها في قائمة المشتريات واعرض الأصناف الموجودة في حقل "items".
...
${JSON.stringify(contextData, null, 2)}`;
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: messages,
        temperature: 0.1, // تقليل الحرارة لأقصى درجة لضمان الدقة ومنع التكرار
      })
    });
