...
export default function DebtsPage() {
  ...
  const handleSettleDebt = async () => {
    ...
    const newRemainingAmount = previousRemaining - amount
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full" dir="rtl">
            ...
            <TabsContent value="customers" className="space-y-4 outline-none animate-in fade-in duration-300">
              ...
              {filteredCustomerDebts.length === 0 ? (
                <div className="text-center py-20 text-muted-foreground opacity-50 font-bold">لا توجد ديون مسجلة</div>
              ) : (
                    ...
                    >
                      <div className="flex gap-4 items-center text-right">
                        <div className={cn(
                          ...
                          isSettled ? "bg-muted text-muted-foreground" : "bg-primary/10 text-primary"
                        )}>
                      ...
                      <div className="flex flex-col items-end gap-1">
                        {isSettled ? (
                          <Badge className="bg-green-100 text-green-700 border-none font-black text-[9px] px-2 py-0.5 rounded-lg flex items-center gap-1">
            ...
            <TabsContent value="suppliers" className="space-y-4 outline-none animate-in fade-in duration-300">
              ...
              {filteredSupplierDebts.length === 0 ? (
                <div className="text-center py-20 text-muted-foreground opacity-50 font-bold">لا توجد مديونيات مسجلة</div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex gap-4 items-center text-right">
                        <div className={cn(
                          "w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner",
                          isSettled ? "bg-muted text-muted-foreground" : "bg-red-50 text-red-500"
                        )}>
              ...
              {filteredHistory?.length === 0 ? (
                ...
                filteredHistory?.map((tr: any) => (
                  <div key={tr.id} className={cn(
                    "p-4 rounded-[1.5rem] border shadow-sm space-y-2.5 transition-all relative overflow-hidden bg-white",
                    tr.type === 'customer_payment' ? 'border-green-100' : 'border-orange-100'
                  )}>
                    <div className="flex justify-between items-center">
                      <Badge className={cn(
                        "text-[9px] font-black px-2 py-0.5 rounded-lg border-none shadow-sm",
                        tr.type === 'customer_payment' ? 'text-white bg-green-600' : 'text-white bg-orange-600'
                      )}>
                    ...
                    <div className="flex justify-between items-center px-1">
                      <div className="flex items-center gap-2 max-w-[60%]">
                        ...
                        <span className="text-sm font-black text-foreground/90 truncate">{tr.entityName}</span>
                      </div>
                    ...
                    {tr.notes && (
                      <p className="text-[10px] text-muted-foreground bg-muted/30 p-2 rounded-xl border border-dashed flex items-center gap-2">
                        <Info className="w-3 h-3" /> {tr.notes}
                      </p>
                    )}
                  </div>
                ))
        ...
        <SheetContent side="bottom" className="h-[90vh] rounded-t-[2.5rem] p-0 overflow-hidden border-none shadow-2xl [&>button]:hidden">
          <SheetHeader className="p-6 bg-primary text-white relative">
            ...
            <div className="flex flex-col gap-1 items-start mt-4 text-right w-full pr-2" dir="rtl">
              ...
              <p className="text-xs text-white/70 font-bold">كشف العمليات والديون الموثقة</p>
            </div>
          </SheetHeader>
          ...
          <div className="h-full overflow-y-auto p-4 space-y-4 pb-32 bg-muted/20" dir="rtl">
            {entityTransactions.length > 0 ? (
              entityTransactions.map((tr: any) => {
                ...
                return (
                  <div key={tr.id} className={cn(
                    "p-5 bg-white rounded-[2rem] border border-border/60 shadow-md space-y-4 relative overflow-hidden transition-all",
                    isPaid && "bg-muted/40 opacity-80"
                  )}>
                    {isPaid && (
                      <div className="absolute top-0 right-0 p-1.5 bg-green-500 text-white rounded-bl-2xl z-10 flex items-center gap-1 px-3 shadow-md">
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        <span className="text-[10px] font-black">مُسددة بالكامل</span>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex flex-col gap-1.5 text-right">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {(tr.items && tr.items.length > 0) ? tr.items.slice(0, 3).map((item: any, idx: number) => (
                            <span key={idx} className="text-[10px] px-2 py-0.5 bg-muted rounded-full font-bold text-muted-foreground flex items-center gap-1 border">
                              <Fish className="w-3 h-3" /> {item.fishType}
                            </span>
                          )) : (
                      ...
                      <div className="flex flex-col items-end gap-1.5">
                        <Badge className={cn(
                          "rounded-xl px-3 py-0.5 text-[9px] font-black border-none shadow-sm",
                          tr.trType === 'sale' ? "bg-green-600 text-white" : (tr.trType === 'purchase' ? "bg-orange-600 text-white" : "bg-accent text-white")
                        )}>
                          {tr.trType === 'sale' ? "مبيعات" : tr.trType === 'purchase' ? "مشتريات" : "مصروف"}
                        </Badge>
                        <div className="flex items-center gap-1.5 text-muted-foreground">
                          <span className="text-[10px] font-bold tabular-nums">{date ? format(new Date(date), "dd/MM/yyyy", { locale: ar }) : "-"}</span>
                    ...
                    <div className="grid grid-cols-3 gap-3 py-3 border-y border-dashed border-border/50 text-center bg-muted/20 rounded-2xl">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] text-muted-foreground font-black">المتبقي</span>
                        <span className={cn("text-sm font-black tabular-nums", isPaid ? "text-muted-foreground line-through" : "text-destructive")}>
                          {tr.remainingAmount.toLocaleString('en-US')}
                        </span>
                      </div>
                      <div className="flex flex-col gap-0.5 border-x border-border/30 px-1">
                        <span className="text-[9px] text-muted-foreground font-black">المدفوع</span>
                        <span className={cn("text-sm font-black tabular-nums text-green-700")}>
                          {tr.paidAmount?.toLocaleString('en-US')}
                        </span>
                      </div>
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] text-muted-foreground font-black">الإجمالي</span>
                        <span className="text-sm font-black tabular-nums">{(tr.totalAmount || tr.amount)?.toLocaleString('en-US')}</span>
                      </div>
                    </div>
                    ...
                    {payments.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-[10px] font-black text-muted-foreground flex items-center gap-2 px-1">
                          <History className="w-3.5 h-3.5 text-primary" /> سجل السداد للفاتورة:
                        </p>
                    ...
                    <div className="flex gap-3 pt-2">
                      <Button 
                        variant="outline" 
                        className="flex-1 h-11 rounded-2xl text-xs font-black gap-2 border-primary/20 text-primary hover:bg-primary/5"
                      ...
                      {!isPaid && (
                        <Button 
                          className="flex-1 h-11 rounded-2xl text-xs font-black gap-2 lux-gradient shadow-lg"
                          onClick={() => {
                            setPaymentTarget(tr);
                            setPaymentAmount(tr.remainingAmount.toString());
                          }}
                        >
                          <Banknote className="w-4 h-4" /> تسجيل سداد
                        </Button>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 opacity-40 bg-white rounded-[3rem] border-2 border-dashed mx-2">
                <CheckCircle2 className="w-16 h-16 text-green-600" />
                <div className="space-y-1">
                  <p className="text-lg font-black">لا توجد ديون موثقة</p>
                  <p className="text-xs font-bold">جميع العمليات لهذه الجهة تمت نقداً أو تم تسويتها بالكامل</p>
                </div>
              </div>
            )}
          </div>
        ...
        <DialogContent className="max-w-[95%] rounded-[2.5rem] mx-auto p-0 overflow-hidden border-none shadow-2xl [&>button]:left-6 [&>button]:right-auto">
          ...
          <div className="p-6 space-y-6" dir="rtl">
            <div className="grid grid-cols-2 gap-4 p-5 bg-muted/40 rounded-3xl border border-dashed text-right border-primary/20">
              <div className="space-y-1">
                ...
                <p className="text-lg font-black text-destructive tabular-nums">{paymentTarget?.remainingAmount?.toLocaleString('en-US')} ر.ي</p>
              </div>
              <div className="space-y-1">
                ...
                <p className="text-xs font-black text-primary truncate bg-white px-2 py-0.5 rounded-lg inline-block border">
                  {paymentTarget?.invoiceNumber || (paymentTarget?.trType === 'expense' ? 'مصروف' : '-')}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2.5 text-right">
              <Label className="text-sm font-black mr-1 flex items-center gap-2 text-primary">
                ...
                المبلغ المراد سداده الآن (بالإنجليزي)
              </Label>
              <Input 
                type="text" 
                inputMode="decimal"
              ...
              {parseFloat(paymentAmount) > paymentTarget?.remainingAmount && (
                <p className="text-[10px] text-destructive font-black flex items-center gap-1.5 justify-end mt-2 animate-pulse">
                  <AlertCircle className="w-4 h-4" /> خطأ: المبلغ المدخل أكبر من المتبقي في هذه الفاتورة
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 gap-5">
               ...
               <div className="space-y-2 text-right">
                <Label className="text-xs font-black mr-1 flex items-center gap-2 text-muted-foreground">
                  ...
                  اسم المستلم / ملاحظات التوثيق
                </Label>
                <Textarea 
                  placeholder="مثال: تم تسليم الدفعة للأخ فلان بن فلان..." 
                  className="rounded-[1.5rem] resize-none text-right h-28 border-muted-foreground/20 focus:ring-primary" 
                  value={paymentNotes} 
          ...
          <DialogFooter className="p-6 bg-muted/30 border-t flex flex-row gap-4">
            ...
            <Button 
              ...
              disabled={submittingPayment || !paymentAmount || parseFloat(paymentAmount) > (paymentTarget?.remainingAmount || 0)}
            >
