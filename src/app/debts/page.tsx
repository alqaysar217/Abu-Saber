...
import { collection, query, doc, updateDoc, addDoc, serverTimestamp, orderBy } from "firebase/firestore"
...
export default function DebtsPage() {
  ...
  const customerDebts = useMemo(() => {
    ...
    return customers.map(cust => {
      ...
      if (entityInvoices.length === 0 && totalRemaining <= 0) return null
      return {
        ...cust,
        totalRemaining,
    ...
    }).filter(Boolean).sort((a: any, b: any) => b.totalRemaining - a.totalRemaining)
  }, [invoices, customers])
  ...
  const supplierDebts = useMemo(() => {
    ...
    return suppliers.map(sup => {
      ...
      if (entityPurchases.length === 0 && entityExpenses.length === 0 && totalRemaining <= 0) return null
      return {
        ...sup,
        totalRemaining,
        transactions: [
  ...
  const totalDebtsToMe = customerDebts.reduce((acc, curr: any) => acc + curr.totalRemaining, 0)
  const totalDebtsByMe = supplierDebts.reduce((acc, curr: any) => acc + curr.totalRemaining, 0)
  ...
  const handleSettleDebt = async () => {
    ...
    try {
      ...
      setPaymentAmount("")
      setPaymentNotes("")
      // Update selected entity data for the UI
      if (selectedEntity) {
         const updatedTransactions = selectedEntity.transactions.map((t: any) => 
           t.id === paymentTarget.id ? { ...t, paidAmount: newPaidAmount, remainingAmount: newRemainingAmount } : t
         )
         setSelectedEntity({
           ...selectedEntity,
           totalRemaining: selectedEntity.totalRemaining - amount,
    ...
    <div className="flex flex-col min-h-screen bg-background pb-24">
      <header className="p-6 bg-white border-b sticky top-0 z-30 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card className="border-none bg-green-50/50 border border-green-100 rounded-2xl shadow-none">
            <CardContent className="p-4 flex flex-col items-center gap-1">
              <span className="text-[10px] font-bold text-green-700 flex items-center gap-1">
                <ArrowUpRight className="w-3 h-3" /> ديون لك
              </span>
              <span className="text-lg font-black text-green-800 tabular-nums">
                {totalDebtsToMe.toLocaleString('en-US')}
              </span>
            </CardContent>
          </Card>
          <Card className="border-none bg-red-50/50 border border-red-100 rounded-2xl shadow-none">
            <CardContent className="p-4 flex flex-col items-center gap-1">
              <span className="text-[10px] font-bold text-red-700 flex items-center gap-1">
                <ArrowDownRight className="w-3 h-3" /> ديون عليك
              </span>
              <span className="text-lg font-black text-red-800 tabular-nums">
                {totalDebtsByMe.toLocaleString('en-US')}
              </span>
            </CardContent>
          </Card>
        </div>
        ...
        {isLoading ? (
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full" dir="rtl">
            <TabsList className="grid w-full grid-cols-2 h-14 rounded-2xl p-1 mb-6 bg-muted/50 border border-border/50 shadow-inner">
              <TabsTrigger value="customers" className="rounded-xl font-black text-[11px] h-full transition-all data-[state=active]:bg-gradient-to-br data-[state=active]:from-[#123524] data-[state=active]:to-[#236045] data-[state=active]:text-white">العملاء (دين لك)</TabsTrigger>
              <TabsTrigger value="suppliers" className="rounded-xl font-black text-[11px] h-full transition-all data-[state=active]:bg-gradient-to-br data-[state=active]:from-[#123524] data-[state=active]:to-[#236045] data-[state=active]:text-white">الموردين (دين عليك)</TabsTrigger>
            </TabsList>
              ...
              {filteredList.length > 0 ? (
                filteredList.map((entity: any) => (
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-5 bg-white rounded-[2rem] border border-border/40 shadow-sm active:scale-[0.98] transition-all hover:shadow-md group"
                  >
                    <div className="flex items-center gap-4">
                      <div className={cn(
                        "p-3 rounded-2xl transition-colors",
                        activeTab === "customers" ? "bg-green-50 text-green-600 group-hover:bg-green-100" : "bg-red-50 text-red-600 group-hover:bg-red-100"
                      )}>
                        <User className="w-6 h-6" />
                      </div>
                      <div className="flex flex-col text-right">
                        <span className="font-black text-sm text-foreground">{entity.name}</span>
                        <span className="text-[10px] text-muted-foreground font-bold flex items-center gap-1">
                          <History className="w-3 h-3" />
                          {entity.transactions?.length || 0} فواتير آجلة
                        </span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      ...
                      {entity.totalRemaining <= 0 ? (
                        ...
                        <span className="text-[9px] font-bold text-muted-foreground uppercase opacity-60">ر.ي</span>
                      )}
                    </div>
                  </button>
                ))
                ...
                <div className="flex flex-col items-center justify-center py-20 opacity-30 text-center space-y-4">
                  ...
                  <p className="font-bold text-sm">لا توجد سجلات ديون حالياً</p>
                </div>
              )}
            </div>
          </Tabs>
      ...
      <Sheet open={!!selectedEntity} onOpenChange={() => setSelectedEntity(null)}>
        <SheetContent side="bottom" className="h-[90vh] rounded-t-[3rem] p-0 overflow-hidden border-none shadow-2xl [&>button]:left-6 [&>button]:right-auto">
          <SheetHeader className="p-8 lux-gradient text-white text-right relative overflow-hidden">
             <div className="absolute top-0 left-0 w-full h-full opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] pointer-events-none"></div>
             <div className="flex justify-between items-start pt-4 relative z-10" dir="rtl">
                <div className="flex items-center gap-4">
             ...
             <div className="mt-8 flex items-center justify-between bg-white/10 p-5 rounded-[1.5rem] border border-white/10 shadow-inner relative z-10">
                <div className="flex flex-col text-right">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-24 bg-background">
            <h3 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2" dir="rtl">
            ...
            <div className="space-y-4" dir="rtl">
              {selectedEntity?.transactions?.map((tr: any) => {
                ...
                const currentRem = (tr.remainingAmount !== undefined ? tr.remainingAmount : ((tr.totalAmount || 0) - (tr.paidAmount || 0)))
                const isPaid = currentRem <= 0
                
                return (
                  <div key={tr.id} className="p-5 bg-white rounded-[2rem] border border-border/40 shadow-sm space-y-4 hover:border-primary/20 transition-all">
                    <div className="flex justify-between items-start">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                           ...
                           <span className="text-[10px] font-bold text-primary truncate max-w-[100px]">{campaign?.name || "حملة عامة"}</span>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 p-3 bg-muted/30 rounded-2xl border border-dashed">
                      <div className="flex flex-col">
                        <span className="text-[8px] font-black text-muted-foreground uppercase">الواصل (المدفوع)</span>
                      ...
                      <div className="flex flex-col border-r border-border/60 pr-3">
                        <span className="text-[8px] font-black text-muted-foreground uppercase">المتبقي (الدين)</span>
                        <span className="text-xs font-black text-destructive tabular-nums">{currentRem.toLocaleString('en-US')}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-1">
                      <Button 
                        variant="outline" 
                        className="flex-1 h-11 rounded-2xl text-[11px] font-black gap-2 border-primary/10 text-primary hover:bg-primary/5"
                      ...
                      {!isPaid && (
                        <Button 
                          ...
                          onClick={() => {
                            setPaymentTarget(tr);
                            setPaymentAmount(currentRem.toString());
                          }}
                        >
          ...
          <div className="p-6 space-y-6" dir="rtl">
            ...
            <div className="space-y-2.5 text-right">
              ...
              <Input 
                type="text" 
                inputMode="decimal"
                className="h-16 rounded-[1.5rem] text-center text-2xl font-black tabular-nums border-2 border-primary/20 focus:ring-primary shadow-inner" 
                value={paymentAmount}
                onChange={(e) => {
            ...
            <div className="grid grid-cols-1 gap-5">
               <div className="space-y-2 text-right">
                <Label className="text-[11px] font-black mr-1 flex items-center gap-2 text-muted-foreground">
               ...
               <div className="space-y-2 text-right">
                ...
                <Textarea placeholder="اكتب تفاصيل إضافية عن السداد..." className="rounded-[1.5rem] resize-none text-right h-24 border-muted-foreground/20 focus:ring-primary" value={paymentNotes} onChange={(e) => setPaymentNotes(e.target.value)} />
               </div>
            </div>
          </div>
