          ...
          <div className="bg-white rounded-[2rem] border shadow-sm overflow-hidden">
            <Table dir="rtl">
              <TableHeader className="bg-muted/30">
                <TableRow>
                  <TableHead className="text-right font-black text-[10px]">المورد / التاريخ</TableHead>
              ...
              <TableBody>
                {filteredPurchases.map((p) => {
                  const supplier = suppliers?.find(s => s.id === p.supplierId)
                  return (
                    <TableRow key={p.id} className="active:bg-muted/50 transition-colors">
                      ...
                      <TableCell className="text-center">
                        <Badge className={cn(
                          ...
                          p.status === "مدفوعة" ? "bg-green-50 text-green-600" : (p.status === "دين" ? "bg-red-50 text-red-600" : "bg-blue-50 text-blue-600")
                        )}>
                          {p.status}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-left">
                        <Button variant="ghost" size="icon" className="w-8 h-8 rounded-lg" onClick={() => router.push(`/campaigns/${p.campaignId}?tab=purchases`)}>
