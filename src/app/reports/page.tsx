...
export default function ReportsPage() {
  ...
  // 1. Monthly Profit Growth Logic (With Demo Data Fallback)
  const monthlyProfitData = useMemo(() => {
    let results: any[] = []
    
    if (invoices && purchases && expenses && invoices.length > 0) {
      ...
      results = Object.entries(monthlyMap)
        .map(([key, data]) => ({
          key,
    ...
    if (results.length === 0) {
      return [
        { name: 'يناير', profit: 12000000 },
        { name: 'فبراير', profit: 18500000 },
        { name: 'مارس', profit: 14200000 },
        { name: 'أبريل', profit: 22000000 },
        { name: 'مايو', profit: 28600000 },
        { name: 'يونيو', profit: 34500000 },
      ]
    }
    return results
  }, [invoices, purchases, expenses])

  // 2. Customer Debt Concentration (With Demo Data Fallback)
  const customerDebtsData = useMemo(() => {
    let results: any[] = []
    
    if (invoices && customers && invoices.length > 0) {
      ...
      results = Object.entries(debtMap).map(([id, amount]) => {
    ...
    if (results.length === 0) {
      return [
        { name: 'مطعم البحر', value: 8500000 },
        { name: 'فندق القصر', value: 6200000 },
        { name: 'شركة التوريد', value: 4800000 },
        { name: 'مطعم الشاطئ', value: 3100000 },
        { name: 'بوفيه السلام', value: 1200000 },
      ]
    }
    return results
  }, [invoices, customers])

  // 3. Supplier Debt Concentration (With Demo Data Fallback)
  const supplierDebtsData = useMemo(() => {
    let results: any[] = []
    
    if (purchases && expenses && suppliers && (purchases.length > 0 || expenses.length > 0)) {
      const debtMap: Record<string, number> = {}
    ...
    if (results.length === 0) {
      return [
        { name: 'شركة الديزل', value: 5400000 },
        { name: 'مصنع الثلج', value: 3200000 },
        { name: 'مورد المكلا', value: 2800000 },
        { name: 'عمال الشحن', value: 950000 },
        { name: 'صيانة القوارب', value: 450000 },
      ]
    }
    return results
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <Card className="border-none shadow-xl rounded-[2rem] overflow-hidden bg-white">
              <CardHeader className="p-6 pb-2 text-right">
                ...
                <p className="text-[10px] text-muted-foreground font-bold italic">ملاحظة: تظهر بيانات تجريبية حالياً للتوضيح</p>
              </CardHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="border-none shadow-md rounded-[1.5rem] bg-green-50 text-green-700">
                <CardContent className="p-5 flex flex-col gap-1 items-center text-center">
                  ...
                  <span className="text-sm font-black tabular-nums">{invoices?.length ? invoices.reduce((acc, curr) => acc + (curr.totalAmount || 0), 0).toLocaleString() : "92,400,000"}</span>
                </CardContent>
              </Card>
        ...
        <Card className="border-none bg-orange-50 rounded-[2rem] border border-orange-100 p-6 shadow-inner">
          <div className="flex flex-col items-center text-center gap-3">
            ...
            <p className="text-[11px] text-orange-800 font-bold leading-relaxed px-4">
              الرسومات المعروضة حالياً تستخدم بيانات افتراضية لتوضيح الشكل النهائي. ستختفي هذه البيانات وتظهر أرقامك الحقيقية بمجرد بدئك في تسجيل العمليات.
            </p>
          </div>
        </Card>
      </main>
